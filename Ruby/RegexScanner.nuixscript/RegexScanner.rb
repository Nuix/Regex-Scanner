...
dialog.validateBeforeClosing do |values|
	...
	if values["generate_word_lists"] && values["word_list_name_template"] !~ /\{title\}/
		CommonDialogs.showWarning("The word list name template must contain the placeholder {title}.")
		next false
	end
...
if dialog.getDialogResult == true
	...
	generate_word_lists = values["generate_word_lists"]
	word_list_name_template = values["word_list_name_template"]
	...
	ProgressDialog.forBlock do |pd|
		...
		work_proc = Proc.new do |item_match_collection|
			...
			item_match_collection.getMatches.each do |match|
				...
				if generate_word_lists
					semaphore.synchronize {
						word_list_grouped_matches[match.getPatternInfo.getTitle][match.getValue] = true
					}
				end
		...
		# Finalize word lists
		if generate_word_lists
			pd.setSubStatusAndLogIt("Generating word lists...")
			word_list_directory = File.join($current_case.getLocation.getPath,"Stores","User Data","Word Lists")
			java.io.File.new(word_list_directory).mkdirs
			word_list_grouped_matches.each do |title,matches|
				placeholders = {
					"title" => title,
				}
				word_list_name = $su.getFormatUtility.resolvePlaceholders(word_list_name_template,placeholders)
				word_list_file_path = File.join(word_list_directory,"#{word_list_name}.words")
				File.open(word_list_file_path,"w:utf-8") do |word_list_file|
					sorted_uniq_matches = matches.keys.sort
					pd.setSubStatusAndLogIt("  Building word list '#{word_list_name}' with #{sorted_uniq_matches.size} matches")
					word_list_file.puts("# Generated by Regex Scanner at #{Time.now}")
					sorted_uniq_matches.each do |match|
						word_list_file.puts(match)
					end
				end
			end
			$utilities.getWordListStore.getWordListNames
		end
